...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        players = createPlayers(numOfPlayer, minPlayer, maxPlayer, maxNumbOfDie, userInput);    //Create player objects and store them in players list
     ...
     */
    public static List<Player> createPlayers(int numOfPlayer, int minPlayer, int maxPlayer, int maxNumbOfDie, @NotNull Scanner userInput) {
     ...
     * @param mapPath The path to the map image
     */
    public static void setTerritory(@NotNull List<Player> players, @NotNull List<Territory> availableTerritories,
                                    @NotNull List<Territory> finalTerritories, @NotNull Scanner userInput, String mapPath) {
     ...
     * @param mapPath The path to the map image
     */
    public static void setTerritory(@NotNull Player player, @NotNull List<Player> players, @NotNull List<Territory> availableTerritories,
                                    @NotNull List<Territory> finalTerritories, @NotNull Scanner userInput, String mapPath) {
        //Set territory stage
        ...
        for (int i = 0; i < players.size(); i++) {
            while (!players.get(i).isLost()) {
                ...
                if (!input.equals("Yes") && !input.equals("yes") && !input.equals("No") && !input.equals("no")) {
                    System.out.println("You have to answer either Yes or No.");
                    continue;
                } else if (input.equals("No") || input.equals("no")) break;
    ...
    public static int play(Player attacker, Territory attackerTerritory, Player defender, Territory defenderTerritory, Scanner userInput) {
        ...
        int result = -2;
        while (numbOfAttackArmy == 0) {
            System.out.print("Enter number of army you want to use to attack " + defenderTerritory.getTerritoryName() + ": ");
            if (userInput.hasNextInt()) {
                numbOfAttackArmy = userInput.nextInt();
                userInput.nextLine();
            }
            Battle atk = new Attack(attacker, attackerTerritory, defenderTerritory, numbOfAttackArmy,
                    numbOfAttackerSpareArmy, numbOfAttackerPenaltyArmy, numbOfDefenderPenaltyArmy);
            if (atk.startBattle()) {
                Battle def = new Defend(defender, defenderTerritory, attackerTerritory, numbOfDefendArmy,
                ...
                if (def.startBattle()) {
                    ...
                    System.out.println("Invalid number of army on " + defenderTerritory.getTerritoryName() + ".");
                    continue;
                }
            }
            else {
                System.out.println("Insufficient number of army on " + attackerTerritory.getTerritoryName() + ".");
                ...
                else System.out.println("Number of attack army must be larger than or equal to " + numbOfAttackerSpareArmy +
                        " and less than " + attackerTerritory.getNumbOfArmy() + ".");
                continue;
            }
        }
        return result;
     ...
     */
    @Nullable
    public static Territory findTerritory(String territoryName ,int territoryIndex, @NotNull List<Territory> territories) {
        Territory foundTerritory = null;
